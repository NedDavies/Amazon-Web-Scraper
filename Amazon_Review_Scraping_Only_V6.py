...
def webscrape():
    ...
    ReviewNumber = Label(root, text=str(len(reviews))+"Reviews Found", bg='white')
    ...
    # get star image
    Review = Image.open('Images & Fonts/Am Rachel.PNG')
    ...
    star3 = Image.open('Images & Fonts/Am Charlene.PNG')
    area = (91,57,111,78)
    EmptyStar = star3.crop(area)
    EmptyStar
    
    path = r'Reviews'+str(Title) 
    if not os.path.exists(path):
        os.makedirs(path)

    for i in range(len(AmazonReviews)):
        #open blank amazon review image
        img = Image.open("Images & Fonts/Amrev blank.png")
        draw = ImageDraw.Draw(img) # rename draw command
        # font = ImageFont.truetype(<font-file>, <font-size>)
        font = ImageFont.truetype("Images & Fonts/AmazonEmber_Rg.ttf", 16) #instantiate amazon ember font
        ...
        draw.text((116+12,34), AmazonReviews['Title'][i],(53,53,53),font=ImageFont.truetype("Images & Fonts/Amazon Ember Bold.ttf", 16))
        draw.text((18,82), AmazonReviews['Dates'][i],(95,95,95),font=ImageFont.truetype("Images & Fonts/AmazonEmber_Rg.ttf", 16))
        draw.text((70,15), AmazonReviews['Reviewer'][i], (0,0,0), font=ImageFont.truetype("Images & Fonts/AmazonEmber_Lt.ttf",18))
        img.save(r"Reviews"+str(Title)+"/AmRev"+Title+str(i)+".png");
        
        
        AmazonReviews.to_csv('Amrevs'+Title+'.csv')
        Scraping_Complete = Label(root, text="Done!, the csv of reviews is now available in the file \'Amrevs"+Title+"\'.csv'\nThe images are saved to the folder: Reviews"+Title, bg="white") 
